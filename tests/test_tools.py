...
def test_rotate_points_0_degrees():
    x, y = laserbeamsize.image_tools.rotate_points(1, 0, 0, 0, 0)
    assert np.isclose(x, 1, atol=1e-8)
    assert np.isclose(y, 0, atol=1e-8)

def test_rotate_points_90_degrees():
    x, y = laserbeamsize.image_tools.rotate_points(1, 0, 0, 0, np.pi/2)
    assert np.isclose(x, 0, atol=1e-8)
    assert np.isclose(y, -1, atol=1e-8)

def test_rotate_points_180_degrees():
    x, y = laserbeamsize.image_tools.rotate_points(1, 0, 0, 0, np.pi)
    assert np.isclose(x, -1, atol=1e-8)
    assert np.isclose(y, 0, atol=1e-8)

def test_rotate_points_360_degrees():
    x, y = laserbeamsize.image_tools.rotate_points(1, 0, 0, 0, 2*np.pi)
    assert np.isclose(x, 1, atol=1e-8)
    assert np.isclose(y, 0, atol=1e-8)
...
def test_values_along_line():
    ...
    x, y, z, s = laserbeamsize.image_tools.values_along_line(image, 0, 0, 1, 1, 2)
...
def test_values_along_line_vertical():
    ...
    x, y, z, s = laserbeamsize.image_tools.values_along_line(image, 0, 0, 0, 3, 4)
...
def test_values_along_line_horizontal():
    image = np.array([[0, 1, 2, 3], [4, 5, 6, 7]])
    x, y, z, s = laserbeamsize.image_tools.values_along_line(image, 0, 0, 3, 0, 4)
